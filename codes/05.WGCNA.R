...
##################### won't run it but still works

library(ggplot2)
library(reshape2)
library(gprofiler2)

draw_module_gprof <- function(name, COL) {
    RESULT <- get(name)[[1]]
    gene_list <- RESULT[RESULT$col == COL, 'gene']
    cat("num gene:", length(gene_list), "\n")
    gostres2 <- gost(
        query = gene_list, 
        organism = "mmusculus", 
        ordered_query = FALSE, 
    ...
    res_df <- data.frame(
        term_name = gostres2$result$term_name, 
        p_value = gostres2$result$p_value,
        significance = gostres2$result$significant,
        intersection = gostres2$result$intersection_size,
        term_size = gostres2$result$term_size,
        source = gostres2$result$source,
        highlight = gostres2$result$highlight
    )
    res_df <- res_df[res_df$highlight == TRUE, ]
    cat("Dimensions of result dataframe:", dim(res_df), "\n")
    if (nrow(res_df) > 0) {
        res_df <- res_df[res_df$source %in% c("GO:BP", "GO:CC", "GO:MF", "KEGG", "CORUM"), ]
        all_comb <- res_df[order(res_df$p_value), ]
        all_comb_20filt <- head(all_comb, 20)
        all_comb_20filt$logpval <- -log10(all_comb_20filt$p_value)
        all_comb_20filt <- all_comb_20filt[order(all_comb_20filt$logpval), ]
        all_comb_20filt$term_name <- factor(all_comb_20filt$term_name, levels = all_comb_20filt$term_name)
        all_comb_20filt$source <- factor(all_comb_20filt$source, levels = c('GO:BP', 'GO:MF', 'GO:CC', 'KEGG', "CORUM"))
        go_bar <- ggplot(all_comb_20filt, aes(x = term_name, y = logpval)) +
            geom_bar(stat = 'identity', aes(fill = source), alpha = 0.3) +  
            geom_text(aes(label = paste0(intersection, '/', term_size)), size = 3, vjust = 0.5, hjust = 1.0) + 
            theme_minimal() +          
            coord_flip() +     
            theme(
            ...
            scale_fill_manual(values = c("GO:BP" = "steelblue", "GO:CC" = "orange", "GO:MF" = "forestgreen", 'KEGG' = 'red', "CORUM" = 'grey')) + 
            labs(x = "-log10(pval)", y = "GO term", title = paste0(name, '_', COL))
        ggsave(paste0(plotpath, "05.gprof_", name, "__", COL, ".png"), plot = go_bar, width = 6, height = 6, dpi = 300)
        ggsave(paste0(plotpath, "05.gprof_", name, "__", COL, ".pdf"), plot = go_bar, width = 6, height = 6, dpi = 300)
        return(gostres2$result)
    }
}
